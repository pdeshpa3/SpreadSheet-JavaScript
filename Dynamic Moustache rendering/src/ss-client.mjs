...
export default class SSClient {
  ...
  async updateCell(ssName, cellId, formula) {
    ...
    try {
      //@TODO
      console.log(ssName+" "+cellId+" "+formula);
      const response = await this.axios.patch(`/api/store/`+ssName.ssName+`/`+cellId, formula);
  ...
  async clear(ssName) {
    try {
      //@TODO
       const response = await this.axios.delete(`/api/store/`+ssName.ssName);
       return response.data;
    }
    catch (err) {
      rethrow(err);
  ...
  async delete(ssName, cellId) {
    try {
      //@TODO
       const response = await this.axios.delete(`/api/store/`+ssName.ssName+`/`+cellId);
       return response.data;
    }
    catch (err) {
      rethrow(err);
  ...
  async readFormulas(ssName) {
    try {
      //@TODO
       const response = await this.axios.get(`/api/store/`+ssName.ssName);
       return response.data;
    }
    catch (err) {
